...
players_by_country <- PlayerPersonalData %>%
...
library(viridis)
g <- ggplot(players_map, aes(long, lat, group = group))+
...
# g <- ggplot(players_map, aes(long, lat, group = group))+
# ggiraph::geom_polygon_interactive(color='black',aes(long, lat, group=group, fill=value,
#         tooltip=sprintf("%s<br/>%s",region,value))) +
#   hrbrthemes::theme_ipsum() +
#   colormap::scale_fill_colormap(colormap=colormap::colormaps$copper, reverse = T) +
#   labs(title='Players per country', subtitle='As number by country')
# widgetframe::frameWidget(ggiraph::ggiraph(code=print(g)))


ggplotly(g, hoverinfo = paste0(region,players))
